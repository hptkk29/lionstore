    ...
    <v-container fluid class="pt-0" :class="$device.isMobileOrTablet ? 'px-2' : 'px-12'">
        ...
        <v-card flat class="c-card">
            <v-toolbar flat class="card-toolbar px-4">
                <v-toolbar-title class="font-weight-bold ff-default fs-13" v-text="$t('t_edit_category')"></v-toolbar-title>
                <v-spacer></v-spacer>
                <v-tooltip top>
                    <template v-slot:activator="{ on }">
                        <v-btn v-on="on" icon nuxt exact to="/dashboard/categories">
                            <v-icon>mdi-format-list-bulleted</v-icon>
                        </v-btn>
                    </template>
                    <span v-text="$t('t_categories')"></span>
                </v-tooltip>
            </v-toolbar>
...
<script>
    export default {
        ...
        async asyncData({ app, $axios, params }) {
            ...
            let response = await $axios.post('dashboard/categories/options/edit', { id: params.id })

            // Decode title
            const title = JSON.parse(response.data.translation.title)
			...
			return {
				form: {
                    ...
                    title_tr   : title.tr,
                    title_ua   : title.ua,
                    description: response.data.translation.description,
        ...
        methods: {
            ...
            async update() {
                ...
                // Send request to server
                await this.$axios
                            .post('dashboard/categories/options/update', data, config)
                            .then(response => {
