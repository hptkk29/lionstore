    ...
    <v-container fluid class="pt-0" :class="$device.isMobileOrTablet ? 'px-2' : 'px-12'">
        ...
        <v-card flat class="c-card">
            <v-toolbar flat class="card-toolbar px-4">
                <v-toolbar-title class="font-weight-bold ff-default fs-13" v-text="$t('t_edit_customer')"></v-toolbar-title>
                <v-spacer></v-spacer>
                <v-tooltip top>
...
<script>
    export default {
        ...
        async asyncData({ app, $axios, params }) {
            ...
            let response = await $axios.post('/dashboard/customers/options/edit', { id: params.id })
        ...
        methods: {
            ...
            async update() {
                ...
                // Send request to server
                await this.$axios
                            .post('dashboard/customers/options/update', data)
                            .then(response => {
