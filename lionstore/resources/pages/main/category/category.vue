            ...
            <v-col cols="12" class="mt-7 mb-12">
                <v-row>
                    ...
                    <template v-for="(subcategory, index) in category.subcategories">
                        <v-col cols="3" :key="`mobile-subcategories-${index}`" class="text-center">
                            <nuxt-link :to="`/category/${ category.handler }/${ subcategory.handler }`" class="mb-3">
                                <v-img :src="subcategoryIcon(subcategory)" :alt="subcategoryTitle(subcategory)" :width="50" :height="50" class="mx-auto"></v-img>
                                <span class="fs-10 font-weight-medium mt-3 d-block" :class="$vuetify.theme.dark ? 'white--text' : 'black--text'" v-text="subcategoryTitle(subcategory)"></span>
                            </nuxt-link>
                        </v-col>
                    </template>
                </v-row>
...
export default {
    ...
    async asyncData({ app, $axios, params }) {
        ...
        let response = await $axios.post('category', { handler: params.category })
    ...
    head() {
        return {
            ...
            meta: [
                ...
                // Description
                { hid: 'description', name: 'description', content: this.category.translation.description },
                ...
                // OG Description
                { hid: 'og:description', property: 'og:description', content: this.category.translation.description },

                // OG Image
                { hid: 'og:image', property: 'og:image', content: this.ogimage },

                // OG Url
                { hid: 'og:url', property: 'og:url', content: this.$clientIndex(`category/${ this.category.handler }`) },
                ...
                // Twitter Description
                { hid: 'twitter:description', name: 'twitter:description', content: this.category.translation.description },

                // Twitter Image
                { hid: 'twitter:image', name: 'twitter:image', content: this.ogimage },

                // Twitter Image Alt
                { hid: 'twitter:image:alt', name: 'twitter:image:alt', content: this.category.translation.description },

                // Twitter Card
                { hid: 'twitter:card', name: 'twitter:card', content: 'summary_large_image' },
                
            ],
            link: [ { rel: 'canonical', href: this.$clientIndex(`category/${ this.category.handler }`) }  ]
        }
    },
    ...
    computed: {
        ...
        title() {
            ...
            const title  = JSON.parse(this.category.translation.title)
        ...
        // Get category ogimage
        ogimage() {

            // Check if category has ogimage
            if (this.category.ogimage) {
                
                return this.$clientIndex(`/uploads/${ this.category.ogimage.file_folder }/${ this.category.ogimage.uid }.webp`)
        ...
        breadcrumbs(){
            return [
                ...
                {
                    url: this.$clientIndex(`category/${ this.category.handler }`),
                    text: this.title,
                }
            ]
        }
    ...
    methods: {
        ...
        async filter() {
            ...
            // Set category
            data.handler       = this.category.handler
        ...
        if (process.client) {
            ...
            if (this.$store.state.settings.analytics.snapchat_pixel_id) {
                window.snaptr('track', 'SEARCH',  { 
                    'search_string': this.category.handler
