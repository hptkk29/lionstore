...
export default class MessagePendingQueue {
     ...
     * @param data 
     */
    constructor (data: Partial<MessagePendingQueue>) {
    ...
    public async queue() {
        ...
        // Set a queue
        const queue            = new Queue('admin::pending::message', { redis: Config.get('redis.connections.local') })

        // Add data to queue
        queue.add({ message: this.message, locale: settings_general?.default_language })
        
        // Process the queue
        queue.process(async function (job, done) {

            // Set html message to send it
            const html = await edge.render('emails/admin/message_pending', {
            ...
            await transporter.sendMail({
                ...
                subject: i18n.__('t_email_subject_pending_message', { site: settings_general?.site_title }),
                html   : html
            });
