...
export default class RefundRequestQueue {
     ...
     * @param data 
     */
    constructor (data: Partial<RefundRequestQueue>) {
    ...
    public async queue() {
        ...
        // Set a queue
        const queue            = new Queue('admin::refund::request', { redis: Config.get('redis.connections.local') })

        // Add data to queue
        queue.add({ order: this.order, locale: settings_general?.default_language, reason: this.reason })
        
        // Process the queue
        queue.process(async function (job, done) {

            // Set html message to send it
            const html = await edge.render('emails/admin/refund_request', {
            ...
            await transporter.sendMail({
                ...
                subject: i18n.__('t_email_admin_refund_request', { site: settings_general?.site_title }),
                html   : html
            });
