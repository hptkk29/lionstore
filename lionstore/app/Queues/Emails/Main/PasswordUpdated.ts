...
export default class PasswordUpdatedQueue {
     ...
     * @param data 
     */
    constructor (data: Partial<PasswordUpdatedQueue>) {
    ...
    public async queue() {
        ...
        // Set a queue
        const queue            = new Queue('customer::password::updated', { redis: Config.get('redis.connections.local') })

        // Add data to queue
        queue.add({ email: this.email, locale: settings_general?.default_language, logo: logo })

        // Process the queue
        queue.process(async function (job, done) {

            // Set html message to send to customer
            const html = await edge.render('emails/main/password_updated', {
            ...
            await transporter.sendMail({
                ...
                to     : job.data.email,
                subject: i18n.__('t_email_password_updated_subject', { site: settings_general?.site_title }),
                html   : html
            });
