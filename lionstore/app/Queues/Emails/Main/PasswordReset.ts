...
export default class PasswordResetQueue {
     ...
     * @param data 
     */
    constructor (data: Partial<PasswordResetQueue>) {
    ...
    public async queue() {
        ...
        // Set a queue
        const queue            = new Queue('main::reset::password', { redis: Config.get('redis.connections.local') })

        // Add data to queue
        queue.add({ customer: this.customer, token: this.token, locale: settings_general?.default_language, logo: logo, email: this.email })
        
        // Process the queue
        queue.process(async function (job, done) {

            // Set html message to send to customer
            const html = await edge.render('emails/main/password_reset', {
            ...
            await transporter.sendMail({
                ...
                subject: i18n.__('t_email_password_reset_subject', { site: settings_general?.site_title }),
                html   : html
            });
