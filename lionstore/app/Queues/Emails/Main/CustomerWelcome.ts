...
export default class CustomerWelcomeQueue {
     ...
     * @param data 
     */
    constructor (data: Partial<CustomerWelcomeQueue>) {
    ...
    public async queue() {
        ...
        // Set a queue
        const queue            = new Queue('customer::welcome::email', { redis: Config.get('redis.connections.local') })

        // Add data to queue
        queue.add({ customer: this.customer, locale: settings_general?.default_language, logo: logo })

        // Process the queue
        queue.process(async function (job, done) {

            // Set html message to send to customer
            const html = await edge.render('emails/main/customer_welcome', {
            ...
            await transporter.sendMail({
                ...
                subject: i18n.__('t_email_customer_welcome', { site: settings_general?.site_title }),
                html   : html
            });
