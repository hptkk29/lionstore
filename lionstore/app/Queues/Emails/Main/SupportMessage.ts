...
export default class SupportMessageQueue {
     ...
     * @param data 
     */
    constructor (data: Partial<SupportMessageQueue>) {
    ...
    public async queue() {
        ...
        // Set a queue
        const queue            = new Queue('main::support::message', { redis: Config.get('redis.connections.local') })

        // Add data to queue
        queue.add({ subject: this.subject, email: this.email, locale: settings_general?.default_language, logo: logo })
        
        // Process the queue
        queue.process(async function (job, done) {

            // Set html message to send to customer
            const html = await edge.render('emails/main/support_message', {
            ...
            await transporter.sendMail({
                ...
                to     : job.data.email,
                subject: i18n.__('t_email_thank_u_for_contacting_us', { site: settings_general?.site_title }),
                html   : html
            });
