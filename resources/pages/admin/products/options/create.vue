            ...
            <v-col cols="12" md="7">
                        ...
                        <v-row>
                            ...
                            <v-col cols="12" lg="12">
                                <v-autocomplete dense outlined
                                    ...
                                    :items="categories"
                                    :item-text="item => JSON.parse(item.translation.title)[$i18n.locale]"
                                    item-value="id"
                ...
                <v-card class="c-card mb-8">
                    <v-toolbar flat class="card-toolbar px-4">
                        ...
                        <!-- Exchange rate -->
                        <v-tooltip top v-if="form.price">
                            <template v-slot:activator="{ on, attrs }">
                                <v-btn icon v-on="on" v-bind="attrs" @click="dialogs.exchange = true">
                                    <v-icon :size="22">mdi-deskphone</v-icon>
                                </v-btn>
                            </template>
                            <span v-text="$t('t_exchange_rate')"></span>
                        </v-tooltip>
                        ...
                        <v-row>
                            ...
                            <v-col cols="12" v-if="seoReady">
                                ...
                                <div class="seo-preview-footer">
                                    ...
                                    <span class="seo-preview-price" v-text="$formatMoney(form.price, currency.locale, currency.code)"></span>
                                    <span class="seo-preview-dot">•</span>
            ...
            <v-col cols="12" md="5">
                ...
                <v-card class="c-card mb-8 px-0 pb-4">
                    <v-toolbar flat class="card-toolbar px-4">
                        ...
                        <v-toolbar-title class="fs-11 text-uppercase font-weight-bold" v-text="$t('t_create_product')"></v-toolbar-title>
                        ...
                        <v-row>
                            ...
                            <v-col lg="12">
                                <v-radio-group v-model="form.action" :mandatory="false" class="mt-0">
                                    ...
                                    <!-- Create another -->
                                    <v-radio color="indigo accent-3" value="another">
                                        <template v-slot:label>
                                            <span class="ff-default fs-11 font-weight-bold" v-text="$t('t_create_another_product')"></span>
                                        </template>
                                    </v-radio>
...
export default {
    ...
    async asyncData({ app, $axios }) {
        ...
        let response = await $axios.post('dashboard/products/options/fetch')
    ...
    methods: {
        ...
        async create() {
            ...
            // Send request to server
            await this.$axios
                            .post('dashboard/products/options/create', data, config)
                            .then(response => {
                                // Success
                                this.$message(this.$t('t_product_successfully_created'), 'thumb-up')

                                // Reset errors
                                this.errors = []

                                // Check selected action after creating product
                                switch (this.form.action) {
                                    case 'another':
                                        
                                        // Refresh page to create another product
                                        // We need to refresh page instead of just reset the form 
                                        // So we can prevent in errors in second attempt to create product
                                        this.$router.go()
