...
export default {
    ...
    async asyncData({ app, $axios, params }) {
        ...
        let response = await $axios.post('childcategory', { handler: params.childcategory })

        return {
            productsData : response.data.results.meta,
            products     : response.data.results.data,
            childcategory: response.data.childcategory
        }
    },
    ...
    head() {
        return {
            ...
            meta: [
                ...
                // Description
                { hid: 'description', name: 'description', content: this.childcategory.translation.description },
                ...
                // OG Description
                { hid: 'og:description', property: 'og:description', content: this.childcategory.translation.description },

                // OG Image
                { hid: 'og:image', property: 'og:image', content: this.ogimage },

                // OG Url
                { hid: 'og:url', property: 'og:url', content: this.$clientIndex(`category/${ this.childcategory.category.handler }/${ this.childcategory.handler }`) },
                ...
                // Twitter Description
                { hid: 'twitter:description', name: 'twitter:description', content: this.childcategory.translation.description },

                // Twitter Image
                { hid: 'twitter:image', name: 'twitter:image', content: this.ogimage },

                // Twitter Image Alt
                { hid: 'twitter:image:alt', name: 'twitter:image:alt', content: this.childcategory.translation.description },

                // Twitter Card
                { hid: 'twitter:card', name: 'twitter:card', content: 'summary_large_image' },
                
            ],
            link: [ { rel: 'canonical', href: this.$clientIndex(`category/${ this.childcategory.category.handler }/${ this.childcategory.handler }`) }  ]
        }
    },
    ...
    computed: {
        ...
        title() {
            ...
            const title  = JSON.parse(this.childcategory.translation.title)
        ...
        parentTitle() {
            ...
            const title  = JSON.parse(this.childcategory.category.translation.title)
        ...
        ogimage() {
            ...
            if (this.childcategory.ogimage) {
                
                return this.$clientIndex(`/uploads/${ this.childcategory.ogimage.file_folder }/${ this.childcategory.ogimage.uid }.webp`)
        ...
        breadcrumbs(){
            return [
                ...
                {
                    url: this.$clientIndex(`category/${ this.childcategory.category.handler }`),
                    text: this.parentTitle,
                },
                {
                    url: this.$clientIndex(`category/${ this.childcategory.category.handler }/${ this.childcategory.handler }`),
                    text: this.title,
                },
            ]
        }
    ...
    methods: {
        ...
        async filter() {
            ...
            // Set childcategory
            data.handler       = this.childcategory.handler

            // Send request to server
            await this
                    .$axios
                    .post('childcategory', data)
                    .then(response => {
        ...
        if (process.client) {
            ...
            if (this.$store.state.settings.analytics.snapchat_pixel_id) {
                window.snaptr('track', 'SEARCH',  { 
                    'search_string': this.childcategory.handler
