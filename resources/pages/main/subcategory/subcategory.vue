<template>
    <v-container fluid grid-list-xl>
        ...
        <v-row v-if="$device.isMobileOrTablet">
            ...
            <!-- Mobile categories -->
            <v-col cols="12" class="mt-7 mb-12">
                <v-row>
                    ...
                    <v-col cols="12">
                        <div class="section-title">
                            <v-row align="center" justify="space-between" class="mx-0">
                                <div>
                                    <div class="section-title-h1">
                                        <h1 v-t="'t_childcategories'"></h1>
                                    </div>
                                </div>
                            </v-row>
                        </div>
                    </v-col>
                    <template v-for="(childcategory, index) in subcategory.childcategories">
                        <v-col cols="3" :key="`mobile-childcategories-${index}`" class="text-center">
                            <nuxt-link :to="`/category/${ subcategory.parent.handler }/${ subcategory.handler }/${ childcategory.handler }`" class="mb-3">
                                <v-img :src="childcategoryIcon(childcategory)" :alt="childcategoryTitle(childcategory)" :width="50" :height="50" class="mx-auto"></v-img>
                                <span class="fs-10 font-weight-medium mt-3 d-block" :class="$vuetify.theme.dark ? 'white--text' : 'black--text'" v-text="childcategoryTitle(childcategory)"></span>
                            </nuxt-link>
                        </v-col>
                    </template>
                </v-row>
...
export default {
    ...
    async asyncData({ app, $axios, params }) {
        ...
        let response = await $axios.post('subcategory', { handler: params.subcategory })
    ...
    head() {
        return {
            ...
            meta: [
                ...
                // Description
                { hid: 'description', name: 'description', content: this.subcategory.translation.description },

                // OG Title
                { hid: 'og:type', property: 'og:type', content: false },

                // OG Title
                { hid: 'og:title', property: 'og:title', content: `${ this.title } ${ this.$store.state.settings.title_separator } ${ this.$store.state.settings.site_title }` },

                // OG Site Name
                { hid: 'og:site_name', property: 'og:site_name', content: this.$store.state.settings.site_title },

                // OG Description
                { hid: 'og:description', property: 'og:description', content: this.subcategory.translation.description },

                // OG Image
                { hid: 'og:image', property: 'og:image', content: this.ogimage },

                // OG Url
                { hid: 'og:url', property: 'og:url', content: this.$clientIndex(`category/${ this.subcategory.parent.handler }/${ this.subcategory.handler }`) },

                // Twitter Title
                { hid: 'twitter:title', name: 'twitter:title', content: `${ this.title } ${ this.$store.state.settings.title_separator } ${ this.$store.state.settings.site_title }` },

                // Twitter Description
                { hid: 'twitter:description', name: 'twitter:description', content: this.subcategory.translation.description },

                // Twitter Image
                { hid: 'twitter:image', name: 'twitter:image', content: this.ogimage },

                // Twitter Image Alt
                { hid: 'twitter:image:alt', name: 'twitter:image:alt', content: this.subcategory.translation.description },

                // Twitter Card
                { hid: 'twitter:card', name: 'twitter:card', content: 'summary_large_image' },
                
            ],
            link: [ { rel: 'canonical', href: this.$clientIndex(`category/${ this.subcategory.parent.handler }/${ this.subcategory.handler }`) }  ]
        }
    },
    ...
    computed: {
        ...
        title() {
            ...
            const title  = JSON.parse(this.subcategory.translation.title)
        ...
        parentTitle() {
            ...
            // Decode parent category title
            const title  = JSON.parse(this.subcategory.parent.translation.title)
        ...
        // Get subcategory ogimage
        ogimage() {

            // Check if subcategory has ogimage
            if (this.subcategory.ogimage) {
                
                return this.$clientIndex(`/uploads/${ this.subcategory.ogimage.file_folder }/${ this.subcategory.ogimage.uid }.webp`)
        ...
        breadcrumbs(){
            return [
                ...
                {
                    url: this.$clientIndex(`category/${ this.subcategory.parent.handler }`),
                    text: this.parentTitle,
                },
                {
                    url: this.$clientIndex(`category/${ this.subcategory.parent.handler }/${ this.subcategory.handler }`),
                    text: this.title,
                },
            ]
        }
    ...
    methods: {
        ...
        async filter() {
            ...
            // Set subcategory
            data.handler       = this.subcategory.handler
        ...
        // childcategory icon
        childcategoryIcon(childcategory) {
        ...
        if (process.client) {
            ...
            if (this.$store.state.settings.analytics.snapchat_pixel_id) {
                window.snaptr('track', 'SEARCH',  { 
                    'search_string': this.subcategory.handler
