...
export default class ReviewPendingQueue {
     ...
     * @param data 
     */
    constructor (data: Partial<ReviewPendingQueue>) {
    ...
    public async queue() {
        ...
        // Set a queue
        const queue            = new Queue('admin::review::pending', { redis: Config.get('redis.connections.local') })

        // Add data to queue
        queue.add({ product: this.product, locale: settings_general?.default_language, review: this.review })
        
        // Process the queue
        queue.process(async function (job, done) {

            // Set html message to send it
            const html = await edge.render('emails/admin/review_pending', {
            ...
            await transporter.sendMail({
                ...
                subject: i18n.__('t_email_admin_review_pending', { site: settings_general?.site_title }),
                html   : html
            });
