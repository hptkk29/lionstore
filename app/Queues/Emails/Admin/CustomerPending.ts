...
export default class CustomerPendingQueue {
     ...
     * @param data 
     */
    constructor (data: Partial<CustomerPendingQueue>) {
    ...
    public async queue() {
        ...
        // Set a queue
        const queue            = new Queue('admin::pending::customer', { redis: Config.get('redis.connections.local') })

        // Add data to queue
        queue.add({ customer: this.customer, locale: settings_general?.default_language })
        
        // Process the queue
        queue.process(async function (job, done) {

            // Set html message to send to customer
            const html = await edge.render('emails/admin/customer_pending', {
            ...
            await transporter.sendMail({
                ...
                subject: i18n.__('email_subject_pending_customer', { site: settings_general?.site_title }),
                html   : html
            });
