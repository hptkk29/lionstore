...
export default class CustomerOrderDeliveredQueue {
     ...
     * @param data 
     */
    constructor (data: Partial<CustomerOrderDeliveredQueue>) {
    ...
    public async queue() {
        ...
        // Set a queue
        const queue            = new Queue('customer::order::delivered', { redis: Config.get('redis.connections.local') })
        
        // Add data to queue
        queue.add({ order: this.order, locale: settings_general?.default_language, logo: logo })

        // Process the queue
        queue.process(async function (job, done) {

            // Set html message to send to customer
            const html = await edge.render('emails/main/order_delivered', {
            ...
            await transporter.sendMail({
                ...
                subject: i18n.__('t_email_customer_order_delivered_subject', { site: settings_general?.site_title }),
                html   : html
            });
